"""
Training Script for AURASeg Ablation Study
Paper: AURASeg - Attention Guided Upsampling with Residual Boundary-Assistive Refinement

Supports training V1 (CSPDarknet + SPP) and V2 (CSPDarknet + ASPP-Lite) models.

Usage:
    # Train V1 (SPP baseline)
    python train.py --model v1_base_spp --epochs 50 --batch-size 8
    
    # Train V2 (ASPP-Lite)
    python train.py --model v2_base_assplite --epochs 50 --batch-size 8
    
    # Resume training
    python train.py --model v1_base_spp --resume path/to/checkpoint.pth

All outputs saved to: ABLATION_11_DEC/runs/{model_version}/
"""
...
from models import V1BaseSPP, V2BaseASPPLite
...
from config import get_v1_config, get_v2_config, AblationConfig


class Trainer:
    """
    Trainer class for ablation study models
    ...
    def _build_model(self) -> nn.Module:
        ...
        if self.config.model_version == 'v1_base_spp':
            model = V1BaseSPP(
                in_channels=self.config.model.in_channels,
                num_classes=self.config.model.num_classes
            )
        elif self.config.model_version == 'v2_base_assplite':
            model = V2BaseASPPLite(
                in_channels=self.config.model.in_channels,
                num_classes=self.config.model.num_classes
            )
        else:
    ...
    def _print_info(self):
        ...
        print("\n" + "=" * 60)
        print(f"ABLATION STUDY - {self.config.model_version.upper()}")
        print("=" * 60)
        ...
        print(f"Loss: {self.config.train.loss_type}")
...
def parse_args():
    ...
    parser = argparse.ArgumentParser(description='Train AURASeg Ablation Models')
    
    # Model selection
    parser.add_argument('--model', type=str, default='v1_base_spp',
                        choices=['v1_base_spp', 'v2_base_assplite'],
...
def main():
    ...
    # Get base config
    if args.model == 'v1_base_spp':
        config = get_v1_config()
    elif args.model == 'v2_base_assplite':
